...
.symptoms-cell {
    ...
    line-height: 3.3em; /* Adjust as needed */
...
<div class="card-header page-header d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center mb-4">
    <h1 class="mb-3 mb-md-0">Patients Appointments</h1>
    <div class="d-flex flex-column flex-sm-row align-items-start align-items-sm-center gap-2">
        <!-- Search Box -->
        <div class="search-box position-relative">
            <form method="GET" action="{{ route('doctor.intakeforms') }}">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search patient or symptoms..." class="form-control ps-3 pe-5 rounded">
...
<div class="card-body table-container">
    ...
    <tbody>
      @forelse ($appointments as $index => $item)
        ...
        <tr class="{{ $levelClass }}">
          ...
          <td class="symptoms-cell">{{ $item->symptoms ?? 'N/A' }}</td>
          <td class="text-center">{{ $item->consultation_type ?? 'N/A' }}</td>
          ...
          <td>
              <form action="{{ route('doctor.appointments.updateStatus', $item->appointment_id) }}" method="POST">
                @csrf
                @method('PUT')
            
                <label for="status">Change Status:</label>
                <select name="status" id="status" onchange="this.form.submit()">
                    <option value="pending" {{ $item->appointment_status == 'pending' ? 'selected' : '' }}>Pending</option>
                    <option value="approved" {{ $item->appointment_status == 'approved' ? 'selected' : '' }}>Approved</option>
                    <option value="cancelled" {{ $item->appointment_status == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                    <option value="completed" {{ $item->appointment_status == 'completed' ? 'selected' : '' }}>Completed</option>
                </select>
            </form>
            </td>
  ...
  <div class="mt-3">{{ $appointments->appends(request()->all())->links() }}</div>
